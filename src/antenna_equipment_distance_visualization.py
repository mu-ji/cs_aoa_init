...
def distance_estimation(rx1_data_array, rx1_data_length, c=LIGHTSPEED):
    sdk_ifft_result = []
    sdk_slope_result = []
    my_ifft_result = []
    my_slope_result = []
    for i in range(rx1_data_length):
        ...
        my_slope_result.append(distance)
        if data['distance_data']['ifft'] != 'nan':
            sdk_ifft_result.append(data['distance_data']['ifft'])
        if data['distance_data']['phase_slope'] != 'nan':
            sdk_slope_result.append(data['distance_data']['phase_slope'])
        ...
        range_advanced, ifft_advanced, full_spectrum, peak_index = my_ifft.advanced_bluetooth_range_estimation(i_result, q_result, mask_list, LIGHTSPEED, 8)
        my_ifft_result.append(range_advanced)
    ...
    return np.mean(sdk_ifft_result), np.mean(sdk_slope_result), np.mean(my_ifft_result), np.mean(my_slope_result)
...
for distance in distance_list:
    data = load_numpy_data('antenna_equipment_distance_data/data_{}.npy'.format(distance))  
    ...
    rx1_sdk_ifft, rx1_sdk_slope, rx1_my_ifft, rx1_my_slope = distance_estimation(rx1_data_array, rx1_data_length)
    rx2_sdk_ifft, rx2_sdk_slope, rx2_my_ifft, rx2_my_slope = distance_estimation(rx2_data_array, rx2_data_length)

    rx1_sdk_ifft_list.append(rx1_sdk_ifft)
    rx1_sdk_slope_list.append(rx1_sdk_slope)
    rx1_my_ifft_list.append(rx1_my_ifft)
    rx1_my_slope_list.append(rx1_my_slope)

    rx2_sdk_ifft_list.append(rx2_sdk_ifft)
    rx2_sdk_slope_list.append(rx2_sdk_slope)
    rx2_my_slope_list.append(rx2_my_slope)
    rx2_my_ifft_list.append(rx2_my_ifft)

plt.figure(figsize=(12, 6))
plt.plot(rx1_sdk_ifft_list, 'r-', label='rx1 SDK IFFT')
plt.plot(rx1_my_ifft_list, 'b-', label='rx1 My IFFT')
plt.plot(rx2_sdk_ifft_list, 'r--', label='rx2 SDK IFFT')
plt.plot(rx2_my_ifft_list, 'b--', label='rx2 My IFFT')

plt.plot(rx1_sdk_slope_list, 'g-', label='rx1 SDK Slope')
plt.plot(rx1_my_slope_list, 'y-', label='rx1 My Slope')
plt.plot(rx2_sdk_slope_list, 'g--', label='rx2 SDK Slope')
plt.plot(rx2_my_slope_list, 'y--', label='rx2 My Slope')

plt.plot([i/100 + 10 for i in distance_list], 'black', label='True Distance')
plt.legend()
plt.show()